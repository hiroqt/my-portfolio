...
export default function ProjectDetails({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-foreground selection:text-background">
      <main className="max-w-screen-md mx-auto px-6 py-12 sm:py-24">
        {/* Navigation */}
        <motion.div 
        ...
        >
          <div className="flex gap-3 mb-6 flex-wrap">
            {project.type && (
              <span className="text-[10px] sm:text-xs font-mono tracking-widest uppercase px-3 py-1.5 rounded-full border border-accent/40 text-accent">
                {project.type}
              </span>
            )}
            {project.tags.map(tag => (
              <span key={tag} className="text-[10px] sm:text-xs font-mono tracking-widest uppercase px-3 py-1.5 rounded-full bg-muted/30 text-muted-foreground">
                {tag}
              </span>
            ))}
          </div>
          
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-display font-bold mb-6 text-foreground leading-tight tracking-tight">
            {project.title}
          </h1>
          
          <p className="text-xl sm:text-2xl text-muted-foreground font-serif leading-relaxed mb-8 max-w-prose">
            {project.summary}
          </p>
